	...
	"use strict";var c=[],d=0,e=function(e,f){
		...
		function h(a,b){
			...
			function j(a,b,c){
				...
				if(a.attachEvent){var d=function(){c.call(a)};
				return a.attachEvent("on"+b,d),d}
				}function k(a,b,c){
					a.addEventListener?a.removeEventListener(b,c,!1):a.detachEvent&&a.detachEvent("on"+b,c)}
				function l(a){
					var b,c;return c=a.indexOf("ms")>-1?1:1e3,b="0s"==a?0:parseFloat(a.replace("s",""))*c}
				function m(a,b){return void 0!==a?new RegExp("(\\s|^)"+b+"(\\s|$)").test(a.className):void 0}
				function n(a,b){
					var c,d,e;for(g(a)===!1&&(d=1,a=[a]),d=a.length,e=0;d>e;e++)c=a[e],m(c,b)===!1&&(c.className+=(c.className?" ":"")+b)}
				function o(a,b){
					var c,d,e;for(g(a)===!1?(d=1,a=[a]):d=a.length,e=0;d>e;e++)c=a[e],m(c,b)===!0&&(c.className=c.className.replace
							(new RegExp("(\\s|^)"+b+"(\\s|$)")," ").replace(/^\s+|\s+$/g,""))}
				function p(a,b){
					var c=a.getBoundingClientRect(),d=!1;return b.clientX>=c.left&&b.clientX<=c.right&&b.clientY>=c.top&&b.clientY<=c.bottom&&(d=!0),d}
				function q(a,b,c){
					if("BODY"===b.nodeName)
						return!1;if(a!==b)
							return c=b,q(a,b.parentNode,c);
						if(void 0!==c)
							for(var d=c,e=a.getElementsByTagName(d.nodeName),f=e.length;f--;)
								if(d===e[f])return f+1}
				function r(a){var c=0,d=b.DIRECTION_NONE;
...
!function(){function a(a){g.utils.removeClass(g.activeCaption,"seq-fadein"),g.activeCaption=e[a-1],setTimeout(function(){g.utils.addClass(g.activeCaption,"seq-fadein")},500)}function b(a){if(!g.options.cycle||g.isFallbackMode!==!1)switch(a){case 1:g.utils.removeClass(g.$prev,"seq-fadein"),g.utils.addClass(g.$next,"seq-fadein");break;case void 0:g.utils.removeClass(g.$prev,"seq-fadein");break;case g.noOfSteps:g.utils.addClass(g.$prev,"seq-fadein"),g.utils.removeClass(g.$next,"seq-fadein");break;default:g.utils.addClass(g.$prev,"seq-fadein"),g.utils.addClass(g.$next,"seq-fadein")}}var c=document.getElementById("sequence"),d=document.getElementById("seq-caption"),e=d.children,f={autoPlay:!0,animateCanvas:!1,phaseThreshold:!1,moveActiveStepToTop:!1,preloader:!0,fadeStepWhenSkipped:!1,reverseWhenNavigatingBackwards:!0},g=sequence(c,f);g.animationStarted=function(c){b(c),a(c)},g.ready=function(){b(g.options.startingStepId),a(g.options.startingStepId)},g.utils.addEvent(c,"click",function(a){var b,c;g.isFallbackMode!==!0&&"IMG"===a.target.tagName&&(b=a.target.parentNode.parentNode,c=g.$steps.indexOf(b)+1,g.goTo(c))})}();